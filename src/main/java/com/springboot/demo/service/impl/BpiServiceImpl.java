...
public class BpiServiceImpl implements BpiService {
	...
	public BpiBean getBpiJsonObject() {
		...
		if (!responseEntity.getStatusCode().is2xxSuccessful()) {
			throw new RuntimeException("API returned " + responseEntity.getStatusCode());
		}
	...
	public Map<String, Object> getExchangeObject() {
		...
		result.put("updateTime",
				Date.from(LocalDateTime.parse(bpiBean.getTime().getUpdatedISO(), DateTimeFormatter.ISO_OFFSET_DATE_TIME)
						.atZone(ZoneId.systemDefault()).toInstant()));
		Map<String, Object> exchangeResultMap = bpiBean.getBpi().entrySet().stream().map(Map.Entry::getValue)
				.map(exchange -> {
